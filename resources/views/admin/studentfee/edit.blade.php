...
@section('container')
    <div class="content-wrapper">
        ...
        <div class="container p-5 bg-white">
            ...
            <form action="{{ route('admin.studentfee.update', $studentfee) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                    <label for="receipt_no" class="form-label">Receipt_No</label>
                    <input type="text" class="form-control" name="receipt_no" value="{{ $studentfee->receipt_no }}" id="receipt_no">
                </div>

                <div class="mb-3">
                    <label for="paid_date" class="form-label">Paid_Date</label>
                    <input type="date" class="form-control" name="paid_date" value="{{ $studentfee->paid_date }}"  id="paid_date">
                </div>

                <div class="mb-3">
                    <label class="form-label">Student Name</label>
                    <input type="text" class="form-control" value="{{ $studentfee->student->full_name }}"  readonly>
                </div>

                <div class="mb-3">
                    <label for="pay_amount" class="form-label">Pay_Amount</label>
                    <input type="number" class="form-control" name="pay_amount" value="{{ $studentfee->pay_amount }}"  id="pay_amount" min="0" step="any">
                </div>

                <div class="mb-3">
                    {{-- Displays either the last remaining balance or the full course fee dynamically --}}
                    <label for="pre_balance" class="form-label">Previous Balance</label>                   
                    <input type="number" class="form-control" 
                        value="{{ $studentfee->pre_balance }}"  
                        name="pre_balance" id="pre_balance" >
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" name="remaining_balance" id="remaining_balance"
                        value="{{ $studentfee->remaining_balance }}">
                </div>

                <div class="mb-3">
                    <label for="remarks" class="form-label">Remarks</label>
                    <input type="text" class="form-control" value="{{ $studentfee->remarks }}"  name="remarks" id="remarks">
                </div>

                <div class="mb-3">
                    <label for="received_by" class="form-label">Received_By</label>
                    <input type="text" class="form-control" name="received_by" value="{{ $studentfee->received_by }}"  id="received_by">
                </div>
